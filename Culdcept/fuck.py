...
headval = struct.unpack("<H",f.read(2))[0]
print(headval)
for i in range(0,headval - 2,2):
	print("Val %d = %d" % (i,struct.unpack("<h",f.read(2))[0]))
	
headval2 = struct.unpack("<H",f.read(2))[0]
print(headval2)
for i in range(0,headval2-2,2):
	print("Val %d = %d" % (i,struct.unpack("<h",f.read(2))[0]))
	
print(f.tell())
indata = f.read()
num_pixels = len(indata) / 2
width = 512
