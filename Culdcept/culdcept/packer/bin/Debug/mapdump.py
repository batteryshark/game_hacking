...
def channelSwap(pixeldata,channelcount):
...
outmap = Image.new('RGB', (BLOCK_SZ*w_blocks,h_blocks*BLOCK_SZ), (255, 255, 255))
...
for i in range(0,len(file_offsets)-1):
	...
	for i in range(0,len(img16_data),2):
		img32_data += RGB565toRGB(struct.unpack("<H",img16_data[i:i+2])[0])
	im32 = Image.frombuffer("RGB",(BLOCK_SZ,BLOCK_SZ),img32_data,'raw',"RGB",0,1)
