/*
	Star Wars Voyager Setup Menu Patch for Version e.4
	compile: cc -shared -m32 -w stv_setup.c -w -ldl -lpthread -D_GNU_SOURCE -o ./stv_setup.so
*/
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
...
static unsigned int frame_cooldown_delay = 2500000;
...
int patch_ibutton(int a1){
	*(unsigned int*)0x0805C160 = 3; // This is the good state of the ibutton
}

// Some static patches on init.
void patch_binary()
{
	// Set the iButton Family Code
	unsigned char ibserial[6] = {0xEF,0xEE,0xEE,0xBE,0xAD,0xDE};
	memcpy(0x08DAFBF4,ibserial,6);
	// Patching relative Directories
	UNPROTECT(0x0805A7C9,4096);
	strcpy(0x0805A7C9,"padsp ./stvsnd ./arom/");
	UNPROTECT(0x08057285,4096);
	strcpy(0x08057285,"./textures/");

	UNPROTECT(0x08058380,4096);
	strcpy(0x08058380,"./stv.settings");
	strcpy(0x08058396,"./stv.hiscores");
	strcpy(0x080583AC,"./stv.audits");
	UNPROTECT(0x08057F49,4096);
	strcpy(0x08057F49,"./stv.log");
	
	// Patch out IButton IOCTL Check
	UNPROTECT(0x0804D035,4096);
	memset(0x0804D035,0x90,0x9E);

	// Patch out internal keyboard commands - they just get in the way...
	UNPROTECT(0x0804CFC6,4096);
	memset(0x0804CFC6,0x90,0x6);

	// Function Detours
	if(getenv("STV_FULL") == NULL){
		detour_function(&fullscreen,0x080563D0); // The game defaults to Fullscreen, but that's dumb.
	}

	detour_function(&patch_ibutton,0x0804C1A0); // Patch out the iButton Check - returns true.
	detour_function(&generic_retone,0x0804C430); // Number of lines the IO can register... this might break fullscreen.
	detour_function(&get_frame_number,0x08056B48); // VBLANK
	detour_function(&sound_play,0x08056F90); // Push the sound ourselves because the game is dumb.
}
