 ...
 */
 static unsigned short event_counter;
static unsigned char read_count_15;
void process_io_state_15(){
    printf("SongState: %x\n",*(unsigned char*)0x083EB861);
    printf("m_bUpLoadIndex: %x\n",*(unsigned char*)0x083EB862);
    printf("New SongState = %d\n",writeBuffer.data_buffer[0]);
    int fd;
    
    switch(writeBuffer.data_buffer[0]){
        case 2:
        	memcpy(readBuffer.data_buffer,writeBuffer.data_buffer,0x4000);
            readBuffer.enum_cmd = 3;//writeBuffer.data_buffer[0];
            fd = open("out2.bin",1);
            real_write(fd,writeBuffer.data_buffer,0x4000);
            readBuffer.buffer_size = 0;
            return;
        case 3:
        	memcpy(readBuffer.data_buffer,writeBuffer.data_buffer,0x4000);
        	//memcpy(readBuffer.data_buffer,writeBuffer.data_buffer,sizeof(writeBuffer));
            readBuffer.enum_cmd = 4;//writeBuffer.data_buffer[0];
        ...
        case 4:
        	memcpy(readBuffer.data_buffer,writeBuffer.data_buffer,0x4000);
        	//memcpy(readBuffer.data_buffer,writeBuffer.data_buffer,sizeof(writeBuffer));
            readBuffer.enum_cmd = 5;//writeBuffer.data_buffer[0];
            readBuffer.buffer_size = 96;
            fd = open("out4.bin",1);
            real_write(fd,writeBuffer.data_buffer,0x4000);
            return;
        ...
        case 6:
        	event_counter++;
        	writeBuffer.data_buffer[3] = event_counter & 0xFF;
        	memcpy(readBuffer.data_buffer,writeBuffer.data_buffer,0x4000);
            readBuffer.enum_cmd = writeBuffer.data_buffer[0];
            readBuffer.buffer_size = 0xA00;
            fd = open("out6.bin",1);
            real_write(fd,writeBuffer.data_buffer,0x4000);

            printf("Returned from Case5-6\n");

            return;
        case 8:
			readBuffer.enum_cmd = 2;//writeBuffer.data_buffer[0];
            readBuffer.buffer_size = 0;
            fd = open("out8.bin",1);
